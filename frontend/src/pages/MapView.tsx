  ...
  useEffect(() => {
    const fetchLocations = async () => {
      try {
        ...
        // If userType is 'Artist', we fetch Employer location data, else Artist
        const targetUserType = userType === 'Artist' ? 'Employer' : 'Artist';

        // 1) Fetch the basic location data
        const response = await axios.get(
          `http://localhost:50001/api/locations?userType=${targetUserType}`,
          {
          ...
          fetchedLocations.map(async (loc) => {
            try {
              const profileRes = await axios.get(
                `http://localhost:50001/api/users/profile/${loc.user_id}`,
                { headers: { Authorization: `Bearer ${token}` } }
              );
              return {
                ...loc,
                profile: profileRes.data, // e.g. { fullname, bio, profile_picture }
              };
            } catch (err) {
              console.error('Error fetching profile for user_id=', loc.user_id, err);
              // If error, return loc without a profile
              return loc;
            }
          })
        );
