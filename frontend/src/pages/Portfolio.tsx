...
const Portfolio: React.FC<PortfolioProps> = ({ artistId }) => {
    ...
    const fetchPortfolio = async () => {
        try {
            ...
            const response = await axios.get(`http://localhost:50001/api/portfolios/${artistId}`, {
                headers: {
    ...
    const handleUpload = async (e: React.FormEvent) => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.post('http://localhost:50001/api/portfolios', formData, {
                headers: {
                    Authorization: `Bearer ${token}`,
    ...
    const handleEdit = async (id: number) => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.put(`http://localhost:50001/api/portfolios/${id}`, { description: editMode.description }, {
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.delete(`http://localhost:50001/api/portfolios/${id}`, {
                headers: { Authorization: `Bearer ${token}` },
            });
