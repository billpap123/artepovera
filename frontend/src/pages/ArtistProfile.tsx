...
const UserProfile = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        const response = await axios.get(`http://localhost:50001/api/users/me`, {
          headers: {
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      if (profile.user_type === 'Artist') {
        await axios.post(`http://localhost:50001/api/artists/profile/${artistId}`, formData, {
          headers: {
        ...
        await axios.post(`http://localhost:50001/api/employers/profile/${employerId}`, formData, {
          headers: {
            'Content-Type': 'multipart/form-data',
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
