...
const Navbar = () => {
    ...
    const fetchNotifications = async () => {
      ...
      try {
        const response = await axios.get(
          `http://localhost:50001/api/notifications/${user.user_id}`,
  ...
  const markAsRead = async (notificationId: number) => {
    try {
      await axios.put(`http://localhost:50001/api/notifications/${notificationId}`, {}, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const deleteNotification = async (notificationId: number) => {
    try {
      await axios.delete(`http://localhost:50001/api/notifications/${notificationId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
