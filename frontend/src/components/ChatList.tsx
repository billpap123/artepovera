    ...
    const fetchChats = async () => {
      try {
        ...
        const response = await axios.get(
          `http://localhost:50001/api/chats/user/${storedUser.user_id}`,
          {
        ...
        if (userIds.size > 0) {
          const userResponse = await axios.post(
            `http://localhost:50001/api/users/get-names`,
          ...
          <ChatWindow
            ...
            receiverId={
              chats.find(chat => chat.chat_id === activeChatId)
                ? (user?.user_id === chats.find(chat => chat.chat_id === activeChatId)?.artist_id
                  ? chats.find(chat => chat.chat_id === activeChatId)?.employer_id ?? 0
                  : chats.find(chat => chat.chat_id === activeChatId)?.artist_id ?? 0)
                : 0
            }
          />
        </div>
