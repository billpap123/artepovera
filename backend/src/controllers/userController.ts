...
export const getUserProfile = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // If you want to unify how you respond, you can do something like:
    const baseURL = process.env.BASE_URL || 'http://localhost:50001';

    const formatPicture = (pic: string | null) => pic
      ? `${baseURL}/${pic.replace(/^uploads\/uploads\//, 'uploads/')}`
      : null;
    ...
    res.json({
      ...
      fullname: user.fullname,
      user_type: user.user_type,
      // If user is Artist, you might see artistProfile
      artistProfile: user.artistProfile
        ...
        : null,
      // If user is Employer, you might see employerProfile
...
export const loginUser = async (
  ...
  try {
    ...
    // Return the user data with employer_id if available
    res.json({
      token,
      user: {
        user_id: user.user_id,
